...
const SPONSOR_INCLUDE = { model: Sponsor, as: "sponsor" };
const ORPHAN_INCLUDE = { model: Orphan, as: "orphan" };
...
const create = async (body) => {
  ...
  if (!orphanId || !sponsorId || !monthlySAmount || !startingSDate || !paymentMethod) {
    throw httpError(400, "يرجى تعبئة جميع الحقول المطلوبة");
  }

  if (paymentMethod === "bank_transfer") {
    if (!bankName || !accountNumber || !accountHolderName || !iban) {
      throw httpError(400, "يرجى تعبئة جميع تفاصيل البنك عند اختيار التحويل البنكي");
    }
  }
